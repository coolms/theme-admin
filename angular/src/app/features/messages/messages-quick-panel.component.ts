...
/**
 * Messages quick-panel (ledger #1012) — the right-drawer content for the topbar
 * chat launcher ({@link MessagesQuickAccessComponent}), mirroring the Calendar
 * quick-panel pattern (M1.2.f.4). Two modes:
 *  - LIST: the current user's conversations (labeled by counterpart, live online
 *    dot #1024, unread badge #1017) + a ＋New picker that opens a 1:1 DM ([#1006]).
 *  - THREAD (#1025): tap a row to read + reply INLINE without leaving the page —
 *    recent messages + a quick composer + realtime ([#1010]) + mark-read ([#1017]).
...
@Component({
    ...
    template: `
        <div class="mqp">
            @if (openId()) {
                <!-- THREAD mode (#1025): read + reply inline. -->
                ...
                @if (loading()) {
                    ...
                    @if (hasMore()) {
                        <!-- Inbox paging (#2120): the drawer holds one page, not the
...
export class MessagesQuickPanelComponent implements OnInit {
    ...
    /**
     * Inbox paging (#2120) — the drawer used to render EVERY conversation the
     ...
     * conversation id (#2127) — the same mechanism the full page uses, so the
    ...
    /** The open thread's conversation id, or null in LIST mode (#1025). */
    ...
    /** Thread bucketed into per-day groups for the date separators (#1033). */
    readonly dayGroups = computed<DayGroup<ChatMessageDto>[]>(
        () => groupByDay(this.threadMessages(), m => m.createdAt, this.dtf),
    );

    /** Lazy "load earlier" paging (#1033) — open on the newest page, prepend on scroll-up. */
    ...
    /**
     * Connection-derived ONLINE set (#1024) — the counterparts holding a live
    ...
    /**
     * PUSHED since #2122 — the shared `presence.chat` channel, held open by the
    ...
    /** Fallback list-refresh cadence — used ONLY while realtime is down (#2112). */
    ...
    /** My participant id within the open conversation — marks "my" messages (#1007). */
    ...
    constructor() {
        // Online presence poll (#1024) — re-poll while the drawer is open, and
        // NOT while the tab is hidden (#2107): each tick costs the server one
        ...
        // Realtime for the OPEN thread (#1025): (re)subscribe to its room channel
        ...
        // ⚠️ The LIST kept whatever `ngOnInit` fetched, for as long as the drawer
        // stayed open (#2112). It had neither of the two paths the full page has
        // had since #1021/#1041, so a conversation could go on receiving messages
        ...
        //    (#2106) — the list read is unpaged, so a connected client must not
        ...
        // Realtime (re)connect catch-up (#1041, mirroring the page): when the
    ...
    counterpartName(c: ChatConversationDto | null): string {
        // Shared with the page (#2126). This copy had no self-notes branch, so
        // the notes room read "Conversation" here and "Notes" there.
        return conversationLabel(c, this.meId);
    }

    /** Unread count for a row — the server's number (#2119), else derived (#1017). */
    unreadCount(c: ChatConversationDto): number {
        // Shared with the page (#2126), including the in-flight override (#2127).
        return unreadFor(c, this.meId, this.readSeqOverride()[c.id] ?? 0);
    }

    /** Avatar for a row — counterpart's photo if any, else colored initials (#1013/#1015). */
    ...
    /** Counterpart's presence dot — connection-online ([#1024]) with #1019 away/busy overlaid. */
    ...
    private effectiveStatus(userId: string | null | undefined, manual: string | null | undefined): string | null {
        // Shared with the page (#2126). This copy required a live connection
     ...
     * set on error. A no-op while the pushed channel is live (#2122) — the poll
    ...
    /** Open a thread INLINE in the drawer (#1025) — load + mark read + subscribe live. */
    open(conversationId: string): void {
        ...
        // not here (#2115). At this point `lastSeq` is 0 and the only seq
    ...
    private loadThread(): void {
        ...
        this.hasMoreOlder.set(false);
        // Open on the newest page; older pages load on scroll-up (#1033).
    ...
    /** Scroll-up lazy load (#1033) — prepend the previous page, preserving the anchor. */
    onThreadScroll(): void {
    ...
    /** A realtime nudge arrived — pull anything past our local high-water (#1010). */
    ...
    private applyMessages(incoming: ChatMessageDto[]): void {
        ...
        // — and now there IS a client-side seq to claim (#2115). Mirrors the full
        ...
        // ⚠️ The `excluded` guard is new here (#2127): the page has always refused
     ...
     * its list badge (#1017). `upTo` is the seq this panel HOLDS (#2115), never
    ...
    private markConversationRead(conv: ChatConversationDto, upTo: number): void {
        ...
        // ⚠️ The optimistic cursor goes in the OVERRIDE map, which is what the
        // badge consults (#2127). This used to edit the participant roster
    ...
    private load(): void {
        ...
        // Same window rule as the background refresh (#2120): on first open there
    ...
    /** Append the next page of rows (#2120) — the rules live in `inbox-paging.util`. */
    loadMore(): void {
    ...
    /**
     * Background list refresh (#2112) — same read as {@link load}, but it never
    ...
    private refreshList(): void {
        // Re-read as many rows as are ON SCREEN, not one page (#2120) — a
    ...
    /**
     * Pull anything the OPEN thread missed while the transport was down (#2112).
